<script lang="ts">
  ...
  import { normalizeAddonGroupName, addonGroupLabel } from "@features/catalog";
  import { normalizeIdList } from "@shared/utils/collections";
  ...
  type CustomizationSection = "flavor" | "toppings" | "jalea";
  let openCustomizationSectionByItem = $state<
    Record<number, CustomizationSection | null>
  >({});
  ...
  function isCustomizationSectionOpen(
    itemIndex: number,
    section: CustomizationSection,
    defaultOpen = false,
  ): boolean {
    const current = openCustomizationSectionByItem[itemIndex];
    if (current === undefined) return defaultOpen;
    return current === section;
  }

  function setOpenCustomizationSection(
    itemIndex: number,
    section: CustomizationSection | null,
  ) {
    openCustomizationSectionByItem = {
      ...openCustomizationSectionByItem,
      [itemIndex]: section,
    };
  }
                      ...
                      {#if hasEditableCustomizations(product) && (flavorOptions.length > 0 || toppingOptions.length > 0 || jaleaOptions.length > 0)}
                        {#if flavorOptions.length > 0}
                          <details
                            ...
                            open={isCustomizationSectionOpen(
                              itemIndex,
                              "flavor",
                              true,
                            )}
          ...
          <div id="checkout-form" class="space-y-6 shrink-0">
            ...
            >
              ...
              {#if form.order_type === "en_local"}
                <div
                  class="md:col-span-2 lg:col-span-1 xl:col-span-2 alert bg-info/10 border-info/20 text-info-content rounded-2xl text-sm"
                >
                  <span>El numero de mesa se asigna al llegar al local.</span>
                </div>
              {:else}
