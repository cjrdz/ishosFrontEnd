<script lang="ts">
  ...
  let { open, busy, rejectReason, rejectError, onRejectReasonChange, onConfirm, onClose }: Props = $props();
...
<dialog class="modal" bind:this={dialogRef} onclose={onClose}>
  <div class="modal-box">
    ...
    <textarea
      ...
      placeholder="Motivo"
      value={rejectReason}
      oninput={(event) => onRejectReasonChange((event.currentTarget as HTMLTextAreaElement).value)}
    ></textarea>
    {#if rejectError}
    ...
    <div class="modal-action">
      ...
      <button class="btn btn-error" type="button" onclick={onConfirm} disabled={busy}>Rechazar</button>
    </div>
  </div>
