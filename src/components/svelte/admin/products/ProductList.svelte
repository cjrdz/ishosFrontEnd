  ...
  <div class="card-body gap-4">
    <div class="flex flex-wrap items-center gap-3">
      ...
      <div class="flex items-center gap-1.5 text-sm text-base-content/80 font-medium shrink-0">
        <span class="badge badge-info badge-sm font-semibold rounded-md text-white!">{filteredProducts.length}</span>
        <span>productos</span>
      </div>

      <button class="btn btn-sm btn-outline shrink-0 ml-auto" type="button" onclick={onOpenGlobalFlavors}>
        Sabores globales
      </button>
      <button class="btn btn-sm btn-outline shrink-0" type="button" onclick={onOpenGlobalAddons}>
        Complementos globales
      </button>
      <button class="btn btn-sm btn-primary shrink-0" type="button" onclick={onCreateProduct} disabled={busy}>
        + Crear producto
      </button>
    </div>
      ...
      <table class="table">
        ...
        <tbody>
          {#if filteredProducts.length === 0}
            <tr><td colspan="5" class="text-center py-6 text-base-content/50">No hay productos</td></tr>
          {:else}
