...
import { normalizeIdList, arraysEqualUnordered } from "../../../components/svelte/admin/tabs/utils/list";
...
export function buildCurrentDraftItem(
  ...
  products: Product[],
): { item: ManualOrderItemDraft; error: string } | { item: null; error: string } {
  if (!productId) {
    return { item: null, error: "Selecciona un producto para agregar a la orden" };
  }
  ...
  return {
    item: {
      ...
      included_addon_ids: normalizeIdList([includedToppingId, includedJaleaId].filter((v) => v)),
...
export function addDraftItem(
  ...
  const existingIndex = manualItems.findIndex(
    (item) =>
      ...
      arraysEqualUnordered(item.included_addon_ids, newItem.included_addon_ids) &&
...
export function removeDraftItem(manualItems: ManualOrderItemDraft[], index: number): ManualOrderItemDraft[] {
...
export function updateDraftItemQuantity(
  ...
  return manualItems
    .map((item, idx) => (idx === index ? { ...item, quantity: safeQuantity } : item))
    .filter((item) => item.quantity > 0);
}

/** Generates a stable unique key for a draft item (useful for keying in loops) */
export function draftItemKey(item: ManualOrderItemDraft, index: number): string {
  return [
    item.product_id,
    item.flavor_id || "",
